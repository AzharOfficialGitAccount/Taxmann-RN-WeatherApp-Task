        ...
        <>
            {data[0] ? (<View style={styles.container}>
                ...
                <View style={styles.current}>
                    ...
                    <Text style={{ textAlign: 'left', fontSize: 25, fontFamily: 'Ubuntu-Regular', color: 'darkslategrey' }}>
                        {today.getDate()} {months[today.getMonth()]} | {today.getDay() == 0 ? (<Text>{days[6]}</Text>)
                            : (<Text>{days[today.getDay() - 1]}</Text>)}</Text>
                    <Text style={{ fontSize: 80, color: "darkslateblue", fontFamily: 'Ubuntu-Regular' }}>
                        {data[0].current.temp}<Text style={{ color: 'orange', fontFamily: 'Ubuntu-Regular' }}>
                            &deg;C</Text></Text>
                    ...
                    {setDay.current.map((item) => {
                        ...
                        return (
                            <View key={item} style={{ borderTopColor: 'gray', borderTopWidth: 2, justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', height: 65, fontFamily: 'Ubuntu-Regular' }}>
                                ...
                                <Text style={{ marginRight: 30, fontSize: 25, color: "darkslateblue" }} >{data[0].daily[test].temp.day}
                                    <Text style={{ color: 'orange' }}>&deg;C</Text></Text>
                            </View>
                        )
                    })}
                </View>
